...
// Enum matching StationStatus from backend
enum StationStatus {
  ACTIVE = 'ACTIVE',
  INACTIVE = 'INACTIVE',
...
export function AddStationModal({ isOpen, onClose, onSuccess }: AddStationModalProps) {
  const [formData, setFormData] = useState({
    name: '',
    location: '',
    status: StationStatus.ACTIVE as 'ACTIVE' | 'INACTIVE' | 'MAINTENANCE',
    totalChargingPoint: '',
    availableChargers: '',
  });
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      // Prepare request body matching CreateStationRequest
        ...
        <div className="p-6">
          ...
          <div className="space-y-4" onKeyPress={handleKeyPress}>
            ...
            <div>
              ...
              >
                <option value={StationStatus.ACTIVE}>Hoạt động</option>
                <option value={StationStatus.INACTIVE}>Không hoạt động</option>
                <option value={StationStatus.MAINTENANCE}>Bảo trì</option>
              </select>
            </div>
