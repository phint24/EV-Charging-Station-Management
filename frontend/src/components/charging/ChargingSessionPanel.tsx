...
// (1) SỬA ĐƯỜNG DẪN IMPORT: Thử đi lên 2 cấp để vào 'src' rồi vào 'ui'
import { Card } from '../ui/card';
import { Button } from '../ui/button';
import { Progress } from '../ui/progress';
import { Badge } from '../ui/badge';
// (2) THAY ĐỔI IMPORT: Xóa kiểu 'ChargingSession' giả
// import { ChargingSession } from '../../data/sample'; 
// (3) THAY ĐỔI IMPORT: Sửa đường dẫn API thành đường dẫn tương đối (giữ nguyên)
import { ChargeSessionDto } from '../../services/ChargeSessionAPI'; 
// (4) XÓA IMPORT CSS GÂY LỖI
// import "../../styles/globals.css"

interface ChargingSessionPanelProps {
  // (3) THAY ĐỔI PROP: Yêu cầu kiểu dữ liệu từ API
  session: ChargeSessionDto;
  onStop: () => void;
}

export function ChargingSessionPanel({ session, onStop }: ChargingSessionPanelProps) {
  // (4) THAY ĐỔI STATE: Dùng tên thuộc tính từ ChargeSessionDto
  const [currentSoc, setCurrentSoc] = useState(session.batteryLevel);
  const [currentKwh, setCurrentKwh] = useState(session.energyUsed);
  const [currentCost, setCurrentCost] = useState(session.currentCost);
  ...
  const getTimeRemaining = () => {
    // (5) GIẢ SỬ DTO CÓ 'timeRemaining' (Nếu không, bạn cần thêm nó vào DTO)
  ...
  return (
    <Card className="p-6 rounded-2xl shadow-lg border-2 border-[#0f766e]">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {/* (6) THAY ĐỔI TÊN THUỘC TÍNH */}
            <Badge variant="outline">{session.connectorType}</Badge>
          </div>
          <h3>Charging in Progress</h3>
          <div className="flex items-center gap-2 text-gray-600 mt-1">
            <MapPin className="h-4 w-4" />
             {/* (7) THAY ĐỔI TÊN THUỘC TÍNH (giả sử station là object lồng nhau) */}
            <span className="text-sm">{session.station.name}</span>
          </div>
        </div>
      ...
      {/* Stats Grid (Các giá trị này đã được lấy từ state, nên không cần sửa) */}
